...
xlabel('时间/s');
ylabel('幅度/V');
legend('算法计算出的漂移','人为给的漂移');%第四个图！！！！！！！漂移对比

%% 画图专用
fy=sin(2*pi*10*t);
figure;
plot(t,fy,'Color','k');%第一个图！！！！！！！传感信号
title('传感信号');
xlabel('时间/s');
ylabel('幅度/V');
...
hold on;
plot(t,y+0.02, 'Linewidth', 2);
hold on;
plot(t,fy, 'Linewidth', 2)%第五个图！！！！！！！结果
title('去漂移后的信号');
xlabel('时间/s');
ylabel('幅度/V');
legend('传感信号+漂移','经过去漂移算法后结果','理想传感信号');
