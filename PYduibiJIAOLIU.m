...
%% 漂移产生
%y=randn(1,1010001);
y=load("piaoyiyuanshishujujiaoliu.csv");
t1=-10:0.001:1000; %t就是相当于一个数组[0.01, 0.02, 0.03,..., 9.98, 9.99, 10]
figure
plot(t1,y);

y_1=filter(PYfilter11,y);%漂移频带
figure
plot(t1,y_1);
...
%fenbianlv=awgn(fenbianlu,20,0)/5;%给1加上噪声，求分辨率
fenbianlv=load("zaoshengjiaoliu.csv");
figure;
...
for i = 100:18000 %剔除两端数据
    if(abs(chafen(1,i+1)-chafen(1,i))>=PYchafenmax)
     PYchafenmax =abs(chafen(1,i+1)-chafen(1,i));
    end
end
...
lpFilt3 = designfilt('bandpassiir','FilterOrder',20, ...
         'HalfPowerFrequency1',0.4,'HalfPowerFrequency2',3, ...
         'SampleRate',1000);
...
hold on;
linshi=filter(PYfilter14,f_noise_drift);
plot(linshi(:,300000:320000));
xlabel('采样点数');
ylabel('幅度');
title('数字滤波器');
legend('原始','无相','有相');
...
% hold on;
plot(f_noise_drift(:,300000:320000)-imf(9,:)-imf(10,:));%-imf(9,:)-imf(1,:)-imf(2,:)-imf(3,:)+20);
hold on;
plot(f_noise_drift(:,300000:320000)-wuci);
hold on;
plot(f_noise_drift(:,300000:320000)-liuci);
hold on;
plot(butterworth_result(:,300000:320000),'Linewidth', 2);
hold on;
plot(result+0.005, 'Linewidth', 3);
hold on;
plot(fy(:,300000:320000), 'Linewidth', 2)%第五个图！！！！！！！结果
...
plot(liuci);
hold on;
plot(f_noise_drift(:,300000:320000)-butterworth_result(:,300000:320000));
hold on;
plot(drift);
hold on;
plot(drift_wen);
...
xlabel('采样点数');
ylabel('幅度');
legend('emd','五次','六次','巴特沃斯滤波器','理想信号','本文算法');
