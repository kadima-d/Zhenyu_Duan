...
%y=randn(1,1010001);
t1=-10:0.001:1000;
y = wgn(1,1010001,1)+0.01*t1; %产生一个m行n列的高斯白噪声的矩阵，p以dBW为单位指定输出噪声的强度
...
for i = 1:length(f_LowPass)-1
    if((abs(f_LowPass(1,i+1)-f_LowPass(1,i)))>resolution/1)%resolution为传感器分辨率,被除数为灵敏度
        fc=3;
        lpFilt2333 = designfilt('lowpassiir','FilterOrder',10, ...
         'PassbandFrequency',fc,'PassbandRipple',0.1, ...
         'SampleRate',1000);
        chafenditong=filtfilt(lpFilt2333,f_LowPass);
        while abs(chafenditong(i+1)-chafenditong(i))>=resolution
        lpFilt2333 = designfilt('lowpassiir','FilterOrder',10, ...
         'PassbandFrequency',fc,'PassbandRipple',0.1, ...
         'SampleRate',1000);
        chafenditong=filtfilt(lpFilt2333,f_LowPass);
    ...
    drift_wen(1,i+1)=drift_wen(1,i)+chafenditong(i+1)-chafenditong(i);
    end
...
figure;
plot(f_noise_drift(:,300000:320000)-drift(:,1),'Color','k');%第三个图！！！！！！！加噪声和漂移
title('加噪声和漂移');
xlabel('采样点数');
ylabel('幅度');
%% 把漂移去除
...
plot(f_noise_drift(:,300000:320000)-(f_noise_drift(:,300000)-20));%第五个图！！！！！！！对比
hold on;
plot(y, 'Linewidth', 2);
hold on;
plot(fy, 'Linewidth', 2);
