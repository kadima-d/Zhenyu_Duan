...
lpFilt = designfilt('lowpassiir','FilterOrder',15, ...
         'PassbandFrequency',1.2,'PassbandRipple',0.1, ...
         'SampleRate',1000);
%fvtool(lpFilt)
...
fenbianlv=awgn(fenbianlu,20,0)/1;%给1加上噪声，求分辨率
figure;
...
resolution = mean(resolutionArr)/100;
figure
plot(resolutionArr);
%% 为算法做准备
zhunbei=filtfilt(lpFilt,f_noise_drift);
figure;
plot(t(:,300000:320000),zhunbei(:,300000:320000));%zhunbei按理来说应该是滤波出来的漂移信号
title('低通出来的漂移');
xlabel('时间/s');
ylabel('幅度/V');
hold on;
